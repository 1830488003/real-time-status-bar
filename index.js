...
jQuery(async () => {
    ...
    function replacePlaceholder(uniqueId, replacementHtml, messageId) {
        ...
        console.groupCollapsed(
            `[${extensionName}] AI返回的HTML内容 (点击展开): ${replacementHtml.substring(0, 200)}...`,
        );
        ...
        if (placeholderSpan.length) {
            ...
            if (settings.showRegenerateButton && $container.length) {
                ...
                // 优化后的重新生成逻辑
                $container
                    .find('.rt-status-bar-regenerate-button')
                    .on('click', async function (event) {
                        ...
                        isGenerating = true;
                        const $contentWrapper = $container.find(
                            `#${contentWrapperId}`,
                        );
                        ...
                        try {
                            ...
                            // 3. 处理结果并替换加载状态
                            const generatedHtml =
                                processAiResponse(generationResult);
    ...
    function loadSettings() {
        ...
        $('#rt-status-bar-generate-choices').prop(
            'checked',
            settings.generateChoices,
        );
        $('#rt-status-bar-use-avatar').prop('checked', settings.useAvatar); // 加载新设置
        $('#rt-status-bar-show-regenerate-button').prop(
            'checked',
            settings.showRegenerateButton,
        );
    ...
    function saveSettings(showToast = false) {
        ...
        settings.generateChoices = $('#rt-status-bar-generate-choices').is(
            ':checked',
        );
        settings.useAvatar = $('#rt-status-bar-use-avatar').is(':checked'); // 保存新设置
        settings.showRegenerateButton = $(
            '#rt-status-bar-show-regenerate-button',
        ).is(':checked');
    ...
    async function buildFinalPrompt(messageToProcess) {
        ...
        // 新增：可折叠的日志输出
        console.groupCollapsed(
            `[${extensionName}] 构建的最终Prompt (点击展开): ${finalPrompt.substring(0, 200)}...`,
        );
    ...
    async function handleNewMessage(messageToProcess) {
        ...
        try {
            ...
            if (typeof placeholderMessageId === 'undefined') {
                console.error(
                    `[${extensionName}] 严重错误：无法获取刚创建的占位符消息ID。`,
                );
            }
    ...
    function startPolling() {
        setInterval(async () => {
            ...
            try {
                ...
                const isNotSelf = !lastMessage.message.includes(
                    'class="rt-status-bar-container"',
                );
    ...
    async function init() {
        ...
        try {
            const [
                systemPromptData,
                defaultStatusPromptData,
                avatarPromptGuideData,
                choicePromptGuideData,
            ] = await Promise.all([
            ...
            console.log(`[${extensionName}] 所有提示词模块已成功加载。`);
        } catch (error) {
            console.error(
        ...
        $('#rt-status-bar-generate-choices').on('change', () =>
            saveSettings(false),
        );
        $('#rt-status-bar-use-avatar').on('change', () => saveSettings(false)); // 绑定新开关的事件
        $('#rt-status-bar-show-regenerate-button').on('change', () =>
            saveSettings(false),
        );
